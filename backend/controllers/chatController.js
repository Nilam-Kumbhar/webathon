...
export const getConversations = async (req, res, next) => {
  try {
    ...
    const conversations = getConversationsList(userId);
...
export const getMessages = async (req, res, next) => {
  try {
    ...
    const messages = findMessagesWithSender(conversationKey, { limit, offset });
    const total = countMessages({ conversationKey });
...
export const markAsRead = async (req, res, next) => {
  try {
    ...
    const result = markMessagesAsRead(conversationKey, userId);
    return res.json({
      success: true,
...
export const getUnreadCount = async (req, res, next) => {
  try {
    const count = countUnreadMessages(req.user._id);
